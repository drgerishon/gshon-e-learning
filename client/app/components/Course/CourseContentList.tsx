...
const CourseContentList: FC<Props> = (props) => {
  ...
  let totalCount: number = 0; //total count of videos previous sections
  ...
  return (
    <div
      className={`mt-[15px] w-full ${
        props.isDemo && 'ml-[-30px] sticky top-24 left-0 z-10'
      }`}
    >
      {videoSections.map((section: string, sectionIndex: number) => {
        ...
        //number of videos in the curernt section
        const sectionVideoCount: number = sectionVideos.length;
        const sectionVideoLength: number = sectionVideos.reduce(
            (totalLength: number, item: any) => {
              // Check if item.videoLength is defined and a number
              if (typeof item.videoLength === 'number') {
        ...
        return (
          <div
            className={`${
              !props.isDemo && 'border-b border-[#ffffffbe] pb-2'
            }`}
            key={section}
          >
